$(function () {
  ...
  // $('.slotBtn button').click(function (e) {
  //   OpenPopup()
  // })
  ...
  function OpenPopup(thisObj) { 
    $('.sign_popup').fadeIn()
    $('.sign_popup').removeClass('transform-out').addClass('transform-in')
    // e.preventDefault()
  }


  function closeWindow(thisObj) { 
    thisObj.parents('.sign_popup').fadeOut()
    thisObj.parents('.sign_popup').removeClass('transform-in').addClass('transform-out')
    // e.preventDefault()
  }
...
;(function($, window, document) {
  ...
  rollNumDaq.prototype = {
      ...
      scroNum: function(options){
          ...
          $num_item.css({"transition": "all 4s linear", "-webkit-transition":"all 4s linear"});
