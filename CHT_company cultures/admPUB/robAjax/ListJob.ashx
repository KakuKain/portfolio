...
public class List : IHttpHandler
{
  ...
  public void ProcessRequest(HttpContext context)
  {
    ...
    {
      varD00 = context.Request["d"].ToString().ToLower();
    }
    ...
    for (i = 0; i < arR.Length; i++)
    {
      if (arR[i].Length > 0)
      {
        ...
        {
          // 讀取 職位說明檔
          dbFile = context.Server.MapPath(string.Format("../../upload/system/{0}.txt", varD00.Trim()));
          if (File.Exists(dbFile))
          {
            string strData = readTxtFile(dbFile);
            string[] arR1 = strData.Split('¶');
            string strJOBDESCTC = (arR1.Length > 0 ? arR1[0] : "");
            string strJOBDESCEN = (arR1.Length > 1 ? arR1[1] : "");
            string strJOBDESCJP = (arR1.Length > 2 ? arR1[2] : "");
            if (strLang == "c") { FBDESC = strJOBDESCTC; }
            else if (strLang == "e") { FBDESC = strJOBDESCEN; }
            else if (strLang == "j") { FBDESC = strJOBDESCJP; }
          }
          //else { FBDESC = dbFile + " 找不到!!"; }
          // 輸出結果
          sb1.Append((sb1.Length > 0 ? strSplitM : "") //(sb1.Length > 0 ? strSplitF : "")
            //+ D00 + strSplitM
            + FBNO + strSplitM
